#!/usr/bin/env python3
# =============================================================================
# ES Training Script: 90° Beam Steering (Right/Forward Receiver)
# =============================================================================
"""
Evolution Strategy training for 90° target angle (right/forward receiver).

This script trains an ES agent to steer electromagnetic waves toward the 
right/forward receiver (90°) while suppressing crosstalk to 0° and 180° receivers.
...
Output:
    - Checkpoints saved to ./results/es_outputs/90deg_{timestamp}/
...
def main():
    """Run ES training for 90° target angle."""
    
    print("="*80)
    print("ES TRAINING: 90° BEAM STEERING (RIGHT/FORWARD RECEIVER)")
    print("="*80)
    ...
    print("\nConfiguration:")
    print(f"  Target angle: 90° (right/forward receiver)")
    ...
    # Initialize ES agent for 90° target
    agent = ESAgent(
        target_angle=90,
        config=config,
        output_dir=None  # Auto-generate: results/es_outputs/90deg_{timestamp}
    )
    ...
    print("\n" + "="*80)
    print("✓ 90° training complete. Ready for analysis or ES+NN training.")
    print("="*80)
