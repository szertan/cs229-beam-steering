#!/usr/bin/env python3
# =============================================================================
# ES Training Script: 180° Beam Steering (Top Receiver)
# =============================================================================
"""
Evolution Strategy training for 180° target angle (top receiver).

This script trains an ES agent to steer electromagnetic waves toward the 
top receiver (180°) while suppressing crosstalk to 0° and 90° receivers.
...
Output:
    - Checkpoints saved to ./results/es_outputs/180deg_{timestamp}/
...
def main():
    """Run ES training for 180° target angle."""
    
    print("="*80)
    print("ES TRAINING: 180° BEAM STEERING (TOP RECEIVER)")
    print("="*80)
    ...
    print("\nConfiguration:")
    print(f"  Target angle: 180° (top receiver)")
    ...
    # Initialize ES agent for 180° target
    agent = ESAgent(
        target_angle=180,
        config=config,
        output_dir=None  # Auto-generate: results/es_outputs/180deg_{timestamp}
    )
    ...
    print("\n" + "="*80)
    print("✓ 180° training complete. Ready for analysis or ES+NN training.")
    print("="*80)
