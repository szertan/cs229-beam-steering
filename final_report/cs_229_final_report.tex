...
\caption{\textbf{ES-Single: Optimized rod configurations and electromagnetic field patterns.} Top row: plasma density parameter $\rho \in [0,1]$ for each rod, where $\rho=0$ (light) indicates transparent rods and $\rho=1$ (dark) indicates highly reflective rods with negative permittivity. Bottom row: log-scale electric field intensity $\log_{10}|E_z|^2$ computed via FDFD simulation. Each optimized design creates a distinct scattering pattern that routes incident power from the left source to the corresponding target receiver. Note how the 0$^\circ$ and 90$^\circ$ designs create ``channels'' through the array, while the 180$^\circ$ design uses reflection to redirect power backward. The field patterns confirm successful beam steering with minimal energy leakage to non-target outputs.}
\label{fig:fields}
\end{figure}

\begin{figure}[h]
\centering
\includegraphics[width=0.8\textwidth]{figures/nn_mode_collapse.pdf}
\caption{\textbf{ES+NN: Mode collapse visualization.} The neural network learns to sacrifice 180$^\circ$ steering ($P_{180}=5.5$) to maximize performance at 0$^\circ$ ($P_0=9.8\times10^5$) and 90$^\circ$ ($P_{90}=4.7\times10^5$). Top row: designs generated by the trained network for each target angle. The 0$^\circ$ and 90$^\circ$ designs show structured patterns, while the 180$^\circ$ design appears less organized. Bottom row: resulting field patterns. The 180$^\circ$ field shows no directed energy toward the top receiver---the network has learned that backward steering requires incompatible rod configurations and abandoned it to maximize total reward. This ``mode collapse'' reveals geometric structure in the design space: forward angles (0$^\circ$, 90$^\circ$) share compatible features, while backward steering (180$^\circ$) requires fundamentally different configurations.}
